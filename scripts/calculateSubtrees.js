const mimcSponge = require('circomlib').mimcsponge;
...
function calculateSubtrees(levels, commitments, currentSubtrees) {
    if (typeof currentSubtrees === 'undefined') {
        var nextSubtrees = zeros.slice(0, levels).map(toFieldElement);
    } else {
        nextSubtrees = currentSubtrees.map(toFieldElement);
    }

    for (let i = 0; i < commitments.length; i++) {
        let currentIndex = i,
            currentLevelHash = toFieldElement(commitments[i])
            ;
        for (let j = 0; j < levels; j++) {
            ...
            currentLevelHash = mimcSponge.multiHash([nextSubtrees[j], right]);
            currentIndex /= 2;
        }
    }
    return nextSubtrees;
...
module.exports = calculateSubtrees;